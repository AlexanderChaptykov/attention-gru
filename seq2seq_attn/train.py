...
from seq2seq_attn.dataset import TwoLangs, SOS_token, CustomDs
...
def teacher_force(batch, decoder, decoder_input, decoder_hidden, encoder_outputs, criterion):
    loss = 0
...
def no_teacher_force(batch, decoder, decoder_input, decoder_hidden, encoder_outputs, criterion):
    loss = 0
    for di in range(batch.dec.shape[1] - 1):
        decoder_output, decoder_hidden, attn_weights = decoder(decoder_input, decoder_hidden, encoder_outputs)
...
def batch_train(batch: TwoLangs, encoder: EncoderRNN, decoder: AttnDecoderRNN, encoder_optimizer, decoder_optimizer,
                criterion):
    ...
    if use_teacher_forcing:
        loss = teacher_force(batch, decoder, decoder_input, decoder_hidden, encoder_outputs, criterion)
    else:
        loss = no_teacher_force(batch, decoder, decoder_input, decoder_hidden, encoder_outputs, criterion)
...
def get_collate(max_len):
    def collate_fn(batch: List[TwoLangs], max_len=max_len):
...
def train(input_lang, output_lang, pairs, encoder, decoder, n_iters, batch_size=32, n_epochs=1, plot_every=100,
          learning_rate=0.01):
    start = time.time()
    ...
    for i in range(n_epochs):
        ...
        dl = DataLoader(dataset, batch_size=batch_size, shuffle=False,
                        collate_fn=get_collate(MAX_LENGTH))
        for iteration, batch in enumerate(dl):
            loss = batch_train(batch, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion)
            ...
            # log_resuls(iteration, print_loss_total, print_every, start, n_iters)
    return
