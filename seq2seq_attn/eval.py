...
from seq2seq_attn.train import get_collate


def evaluate(input_lang, output_lang, encoder: EncoderRNN, decoder: AttnDecoderRNN, pair, max_length=MAX_LENGTH):
    two_lang = tensorsFromPair(input_lang, output_lang, pair)
    batch = get_collate(max_length)([two_lang])
...
def evaluateAndShowAttention(input_lang, output_lang, encoder, decoder, pair, file_name):
